...
public class Informacion_equipos extends javax.swing.JFrame {
    ...
    public Informacion_equipos() {
        ...
        try {
            ...
            PreparedStatement pst = cn_1.prepareStatement("select tipo_equipo, marca, modelo, num_serie, dia_ingreso, mes_ingreso, annio_ingreso, observaciones, estatus, ultima_modificacion, comentarios_tecnicos from equipos where id_equipo = '" + Info_Clientes.ID_E + "'");

            ResultSet rs = pst.executeQuery();

            if (rs.next()) {

                jTextField_nameforclient.setText(Gestion_de_Clientes.client);
    ...
    private void initComponents() {
        ...
        jTextPane_Comentariostec.setEditable(false);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        String type_update, marca_update, modelo_update, num_update, obs_update, estatus_update;
        int validacion = 0;
        
        type_update = jComboBox_typeInfo.getSelectedItem().toString();
        marca_update = jComboBox_marcaInfo.getSelectedItem().toString();
        estatus_update = jComboBox_estatusInfo.getSelectedItem().toString();
        
        modelo_update = jTextField_modeloInfo.getText().trim();
        num_update = jTextField_numInfo.getText().trim();
        obs_update = jTextPane_ObsInfo.getText().trim();
        
        if (type_update.equals("")){
             
            validacion++;
            
        }
        
        if (marca_update.equals("")){
             
            validacion++;
            
        }
        
        if (estatus_update.equals("")){
             
            validacion++;
            
        }
        
        if (modelo_update.equals("")){
             
            jTextField_modeloInfo.setBackground(Color.PINK);
            validacion++;
            
        }
        
        if (num_update.equals("")){
             
            jTextField_numInfo.setBackground(Color.PINK);
            validacion++;
            
        }
        
        if (obs_update.equals("")){
        ...
        try {
            ...
            PreparedStatement pst_2 = cn_2.prepareStatement("update equipos set tipo_equipo = ?, marca = ?, modelo = ?, num_serie = ?, observaciones = ?, estatus = ? where id_equipo = '" + Info_Clientes.ID_E + "'");
            
            if (validacion != 0) {
                
                JOptionPane.showMessageDialog(null, "LLene todos los campos requeridos.", "Mensaje", JOptionPane.WARNING_MESSAGE);
                
            }
            
            else {
                
                pst_2.setString(1, type_update);
                pst_2.setString(2, marca_update);
                pst_2.setString(3, modelo_update);
                pst_2.setString(4, num_update);
                pst_2.setString(5, obs_update);
                pst_2.setString(6, estatus_update);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Informacion_equipos.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Informacion_equipos().setVisible(true);
            }
        });
    }
